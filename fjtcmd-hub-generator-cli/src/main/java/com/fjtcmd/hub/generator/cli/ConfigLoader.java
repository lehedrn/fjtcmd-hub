...
import com.fjtcmd.hub.common.constant.GenConstants;
...
import com.fjtcmd.hub.generator.util.GenUtils;
...
public class ConfigLoader
{
     ...
     * 从 YAML 文件加载配置
     */
    public static GenCliConfig load(String configPath) throws IOException
    {
        ...
        {
            return yaml.loadAs(is, GenCliConfig.class);
        }
    }

    /**
     * 将全局配置写入 GenConfig 静态字段（影响 DdlParser 中 GenUtils.initTable 的行为）
     * <p>
     * 必须在 DdlParser.parse() 之前调用。
     */
    public static void applyGlobalToGenConfig(GenCliConfig config)
    {
        if (config == null || config.getGlobal() == null)
        {
            return;
        }
        GenCliConfig.GlobalConfig g = config.getGlobal();
     ...
     */
    public static void applyConfig(List<GenTable> tables, GenCliConfig config)
    {
        if (config == null)
        {
            return;
        ...
        GenCliConfig.GlobalConfig g = config.getGlobal();
        Map<String, GenCliConfig.TableConfig> tableConfigs = config.getTables();
        ...
        for (GenTable table : tables)
        {
            ...
            // --- 第二层：表级覆盖 ---
            GenCliConfig.TableConfig tc = null;
            ...
            // --- 构建 options JSON（parentMenuId / genView / 树表字段） ---
            buildOptions(table, g, tc);

            // --- 第三层：列级覆盖 ---
            Map<String, GenCliConfig.ColumnConfig> colConfigs = (tc != null) ? tc.getColumns() : null;
            if (colConfigs != null && table.getColumns() != null)
            {
                for (GenTableColumn column : table.getColumns())
                {
                    GenCliConfig.ColumnConfig cc = colConfigs.get(column.getColumnName());
                    if (cc != null)
                    {
            ...
            // --- 第四层：归一化 null 值 ---
            // Velocity 模板中使用 "" != $column.dictType 判断，null 会导致判断失效
     ...
     * 应用全局默认值到 GenTable
     */
    private static void applyGlobalDefaults(GenTable table, GenCliConfig.GlobalConfig g)
    {
        if (g == null)
        {
            return;
        ...
        if (g.getFormColNum() != null)
        {
            table.setFormColNum(g.getFormColNum());
        }
    }
     ...
     * 应用表级配置到 GenTable
     */
    private static void applyTableConfig(GenTable table, GenCliConfig.TableConfig tc)
    {
        // -- 基本信息 --
        ...
        // -- 主子表模板配置（新设计） --
        if (tc.getHasSubTable() != null && tc.getHasSubTable())
        {
            table.setHasSubTable(true);
            if (tc.getSubTable() != null)
            {
                GenTable.SubTableConfig subTableConfig = new GenTable.SubTableConfig();
                GenCliConfig.SubTableConfig srcSubTable = tc.getSubTable();
                subTableConfig.setClassName(srcSubTable.getClassName());
                subTableConfig.setBusinessName(srcSubTable.getBusinessName());
                subTableConfig.setSubRoute(srcSubTable.getSubRoute());
                subTableConfig.setFunctionName(srcSubTable.getFunctionName());
                subTableConfig.setFkName(srcSubTable.getFkName());
                subTableConfig.setFkJavaField(srcSubTable.getFkJavaField());
                subTableConfig.setPermissionPrefix(srcSubTable.getPermissionPrefix());
                table.setSubTableConfig(subTableConfig);
            }
        }
        if (tc.getIsSubTable() != null && tc.getIsSubTable())
        {
            table.setIsSubTable(true);
            if (tc.getMainTable() != null)
            {
                GenTable.MainTableConfig mainTableConfig = new GenTable.MainTableConfig();
                GenCliConfig.MainTableConfig srcMainTable = tc.getMainTable();
                mainTableConfig.setClassName(srcMainTable.getClassName());
                mainTableConfig.setBusinessName(srcMainTable.getBusinessName());
                mainTableConfig.setTableName(srcMainTable.getTableName());
                mainTableConfig.setFunctionName(srcMainTable.getFunctionName());
                mainTableConfig.setPkJavaField(srcMainTable.getPkJavaField());
                mainTableConfig.setNameJavaField(srcMainTable.getNameJavaField());
                mainTableConfig.setFkJavaField(srcMainTable.getFkJavaField());
                table.setMainTableConfig(mainTableConfig);
            }
        }
        if (tc.getOrderNum() != null)
        {
            table.setOrderNum(tc.getOrderNum());
        }
    }

    /**
     * 构建 options JSON，供 VelocityUtils 读取
     * <p>
     * 包含：parentMenuId / genView / 树表字段（treeCode / treeParentCode / treeName）
     */
    private static void buildOptions(GenTable table, GenCliConfig.GlobalConfig g, GenCliConfig.TableConfig tc)
    {
        ...
        // genView：表级 > 全局
        Boolean genView = null;
        if (tc != null && tc.getGenView() != null)
        {
            genView = tc.getGenView();
        }
        else if (g != null && g.getGenView() != null)
        {
            genView = g.getGenView();
        }
        if (genView != null)
        {
        ...
        // 菜单排序号
        if (tc != null && tc.getOrderNum() != null)
        {
            options.put("orderNum", tc.getOrderNum());
        }

        // 主子表模板配置
        if (table.isHasSubTable())
        {
            options.put("hasSubTable", true);
        }
        if (table.isIsSubTable())
        {
            options.put("isSubTable", true);
        }
     ...
     */
    private static void applyColumnConfig(GenTableColumn column, GenCliConfig.ColumnConfig cc)
    {
