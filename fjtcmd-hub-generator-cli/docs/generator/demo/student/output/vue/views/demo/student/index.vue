  ...
  <div class="app-container">
    ...
    <el-table ref="tableRef" v-loading="loading" :data="studentList" :height="tableHeight" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="爱好" align="center" prop="studentHobby">
        <template #default="scope">
          <dict-tag :options="biz_student_hobby" :value="scope.row.studentHobby?.split(',') ?? []"/>
        </template>
      </el-table-column>
          ...
          <el-col :span="24">
            <el-form-item label="爱好" prop="studentHobby">
              <el-checkbox-group v-model="form.studentHobby">
                <el-checkbox
                  v-for="dict in biz_student_hobby"
                  :key="dict.value"
                  :label="dict.value">
                  {{dict.label}}
                </el-checkbox>
...
function handleUpdate(row: Student) {
  ...
  getStudent(_studentId).then(response => {
    ...
    form.value.studentHobby = form.value.studentHobby.split(",")
    open.value = true
    title.value = "修改学生信息"
  })
}
...
function submitForm() {
  proxy.$refs["studentRef"].validate((valid: boolean) => {
    if (valid) {
      form.value.studentHobby = form.value.studentHobby.join(",")
