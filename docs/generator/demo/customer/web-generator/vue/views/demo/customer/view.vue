<template>
  <el-drawer title="客户信息表详情" v-model="visible" direction="rtl" size="60%" append-to-body :before-close="handleClose" class="detail-drawer">
...
const open = async (customerId: number): Promise<void> => {
  ...
  try {
    ...
    console.error('获取客户信息表信息失败:', error)
  } finally {
    loading.value = false
  }
}
